...
const DEFAULT_EMAIL_CONFIG: EmailServiceConfig = {
  isEnabled: true,
  provider: 'apps_script', // Proveedor recomendado y gratuito vía Google Apps Script
...
async function dispatchEmail(
  ...
  // Si EmailJS está seleccionado como proveedor activo
  if (config.isEnabled && config.provider === 'emailjs') {
    // 1. Validar que las credenciales mínimas estén provistas
    if (!config.emailjsServiceId?.trim() || !config.emailjsPublicKey?.trim()) {
      const errorMsg = 'Configuración de EmailJS incompleta: Debes ingresar el Service ID y la Public Key en los ajustes.';
      saveSentEmailRecord({
        id: `email-${Date.now()}-${Math.random().toString(36).substr(2, 5)}`,
        toEmail,
        toName,
    ...
    // 2. Resolver el Template ID con fallback inteligente
    // Si es bienvenida busca welcome -> activation. Si es activación o test busca activation -> welcome.
    const resolvedTemplateId =
      (type === 'welcome'
        ? config.emailjsTemplateWelcome || config.emailjsTemplateActivation
        : config.emailjsTemplateActivation || config.emailjsTemplateWelcome
      )?.trim();

    if (!resolvedTemplateId) {
      const errorMsg = 'Configuración de EmailJS incompleta: Debes ingresar al menos un Template ID en los ajustes.';
      saveSentEmailRecord({
        id: `email-${Date.now()}-${Math.random().toString(36).substr(2, 5)}`,
        toEmail,
        toName,
    ...
    try {
      // 3. Preparar parámetros con alias exhaustivos para coincidir con cualquier plantilla de EmailJS
      const finalParams: Record<string, string> = {
        ...
        to_name: toName.trim(),
        name: toName.trim(),
        athlete_name: toName.trim(),
        subject,
        message: templateParams.message_body || bodySnippet,
        message_body: templateParams.message_body || bodySnippet,
        ...templateParams,
      };
      ...
      // Si el usuario configuró clave privada (Access Token para strict security de EmailJS)
      ...
      if (response.ok) {
        ...
        return {
          success: true,
          message: `¡Correo enviado exitosamente vía EmailJS a ${toEmail}!`,
        };
      } else {
        const rawError = await response.text();
        let humanError = rawError;

        if (rawError.includes('The user_id param is required') || rawError.toLowerCase().includes('public key')) {
          humanError = 'Public Key inválida o no encontrada. Verifica en EmailJS (Account > General > Public Key).';
        } else if (rawError.toLowerCase().includes('service') && rawError.toLowerCase().includes('not found')) {
          humanError = `El Service ID "${config.emailjsServiceId}" no existe en tu cuenta de EmailJS.`;
        } else if (rawError.toLowerCase().includes('template') && rawError.toLowerCase().includes('not found')) {
          humanError = `El Template ID "${resolvedTemplateId}" no existe en tu cuenta de EmailJS.`;
        } else if (rawError.toLowerCase().includes('recipient') || rawError.toLowerCase().includes('recipients')) {
          humanError = 'EmailJS requiere destinatario. Revisa en tu plantilla de EmailJS que el campo "To Email" tenga "{{to_email}}".';
        } else if (rawError.toLowerCase().includes('token') || rawError.toLowerCase().includes('forbidden') || response.status === 403) {
          humanError = 'Acceso no autorizado. Si tienes activada la seguridad estricta en EmailJS, ingresa la Clave Privada (Private Key / Access Token) en los ajustes.';
        }

        // Si la plantilla de activación falló en EmailJS pero existe la de bienvenida (que ya funciona):
        // ¡Intentar reenvío automático de rescate con la de bienvenida!
        if (
          type === 'activation' &&
          config.emailjsTemplateWelcome?.trim() &&
          resolvedTemplateId !== config.emailjsTemplateWelcome.trim()
        ) {
          console.warn('La plantilla de activación falló en EmailJS. Intentando reenvío de rescate con plantilla de bienvenida...');
          try {
            const fallbackPayload = {
              ...payload,
              template_id: config.emailjsTemplateWelcome.trim(),
            };
            ...
            if (fallbackRes.ok) {
              saveSentEmailRecord({
                ...
                sentAt: new Date().toISOString(),
                status: 'sent',
                bodySnippet: `${bodySnippet} (Entregado usando plantilla de respaldo de bienvenida)`,
              });
              return {
                success: true,
                message: `¡Correo de activación enviado exitosamente a ${toEmail}! (Se utilizó la plantilla de bienvenida como respaldo porque la de activación reportó error en EmailJS).`,
              };
            }
          } catch (retryErr) {
            console.error('Fallo también el reintento con plantilla de respaldo:', retryErr);
          }
        }

        const formattedError = `Error EmailJS (${response.status}): ${humanError}`;
      ...
      const errorMsg = `Error de conexión al enviar correo: ${err.message || 'Verifica tu conexión a internet'}`;
      saveSentEmailRecord({
        id: `email-${Date.now()}-${Math.random().toString(36).substr(2, 5)}`,
        toEmail,
        toName,
  ...
  // Modo Simulación Automática (Offline / Desarrollo / Producción sin llaves)
